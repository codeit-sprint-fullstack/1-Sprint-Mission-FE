...
export default function ProductImageListSet({ images }: { images: string[] }) {
  const [validImgs, setValidImgs] = useState<string[]>(images);
  ...
  useEffect(() => {
    const loadImages = () => {
      const imgPromises = images.map((img: string) => {
