...
const useFormValidation = (initialValues) => {
  ...
  const validate = () => {
    ...
    // 필드별 유효성 검사
    if (touched.name && (!values.name || values.name.length < 1 || values.name.length > 10)) {
      newErrors.name = '상품명은 1자 이상 10자 이내로 입력해주세요.';
    }

    if (touched.description && (!values.description || values.description.length < 10 || values.description.length > 100)) {
      newErrors.description = '상품 소개는 10자 이상 100자 이내로 입력해주세요.';
    }

    if (touched.price && (!values.price || isNaN(values.price) || values.price <= 0)) {
