...
![image](./page_img/login_modal.png)
![image](./page_img/signup.png)
![image](./page_img/signup_modal.png)
![image](./page_img/g_analytics.png)

## 멘토에게
- 심화 -> 랜딩 페이지 조건의 'PC, Tablet 사이즈의 이미지 크기는 고정값을 사용합니다.' 의 의미가 px로 고정하라는 것이 맞는지 모르겠습니다.
- 로그인(회원가입)시 유효성 검사의 조건이 안맞으면 error 변수에 특정 값이 들어가 버튼 활성화가 안되게 구현했습니다. 근데 이렇게 하니까 유효성 검사 기능이 늘어날 수록 로직이 꼬일 거 같습니다. 로그인 버튼 활성화 기능을 어떻게 구현하는게 효율적인지 궁금합니다. 
